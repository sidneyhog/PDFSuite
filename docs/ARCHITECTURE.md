...
12. ✅ Relatório de escrituras para o escrevente — **não reprocessa**: lê a árvore de saída já gerada (`<base>/<diagnóstico>/<livro>/`) + os `reports/Importacao_livro*.csv` e consolida numa planilha `.xlsx` com abas (Resumo com `DiagnosticoReal` recalculado, Folhas Faltando, Duplicadas, Conflitos com `Situacao`/`AcaoSugerida` cruzadas, Anexos por Folha, Rastreabilidade). Sem `openpyxl`, sai um conjunto de `.csv` (um por aba). `EscrituraRelatorioService` → `EscrituraRelatorioRepository`.
13. ✅ Tratar conflitos e validar a saída — (a) resolve os conflitos (página com código de outro livro, mal arquivada no servidor): se a folha falta no livro certo, copia a página de origem para lá, re-diagnostica e move a pasta se fechou, e marca a linha como `Resolvido` no CSV; casos ambíguos só listados. (b) valida os `Importacao_livro*.csv` contra o disco (folha `Gerada` ausente, arquivo órfão, origem sumida da rede). Nada é apagado. `EscrituraConflitoService` → `EscrituraConflitoRepository`.
